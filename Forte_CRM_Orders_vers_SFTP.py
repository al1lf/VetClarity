...
if __name__ == "__main__":
    ...
    sftp_user = os.getenv('SFTP_USER_FOR')
    sftp_password = os.getenv('SFTP_PASSWORD_FOR')
    
    sftp_folder = os.getenv('SFTP_FOLDER_FOR')
    # remote_path = f'/{sftp_folder}/{os.path.basename(export_file)}'  # Chemin distant sur le serveur SFTP
    remote_path = f'{os.path.basename(export_file)}'  # Chemin distant sur le serveur SFTP
    print(remote_path)
